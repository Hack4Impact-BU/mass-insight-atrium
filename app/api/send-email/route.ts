...
async function processSingleRecipient(
  ...
  try {
    ...
    const recipientData = {
      ...
      person_id: isManualRecipient ? null : (typeof recipient.id === 'number' ? recipient.id : null),
      manual_email: isManualRecipient ? recipientEmail : null,
      status: 'pending'
    };
    ...
    const { data: recipientRecord, error: recipientError } = await supabase
      ...
      .upsert(recipientData, { 
        onConflict: 'campaign_id,manual_email,person_id',
        returning: true 
      })
      .select()
      .single();
